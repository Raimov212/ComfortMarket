...
const LeftFromInvoice = () => {
  const [dataProductInfo, setDataProductInfo] = useState<ProductsDataType[]>(
    []
  );
  ...
  const fetchProduct = async () => {
    setLoading(true);
    try {
      api.get(`/products/by-premise/${storagePremiseID}`).then((res) => {
        console.log("res", res);
        if (res.status === 200) {
  ...
  const handleAddToProduct = (record: ProductsDataType) => {
      ...
      <dialog id="create_premise_for_product_invoice" className="modal">
        <div className="modal-box flex justify-center w-4/12 max-w-5xl relative">
          <SelectPremiseForProductInvoice
            type="FROM_INVOICE"
            storagePremiseID={storagePremiseID}
...
export default LeftFromInvoice;
